...
bool encrypt(fstream &inputFile, fstream &outputFile, const int mode){
    int len;
    int plaintext_len;
    ...
    if(mode ==  ECB){
        if(!EVP_EncryptInit_ex(ctx, EVP_aes_256_ecb(), nullptr, key, iv)){
            cout << "Error: EVP_EncryptInit_ex failed" << std::endl;
            EVP_CIPHER_CTX_free(ctx);
            return false;
        };
    }
    ...
    while(!inputFile.eof()){
        //reads from stream
        inputFile.read((char*)plaintext, 1024);
        //checks amount of characters read and works with that amount
        plaintext_len = (int)inputFile.gcount();
        if(plaintext_len == 0){
            break;
        }
        if(!EVP_EncryptUpdate(ctx, plaintext, &len, plaintext, plaintext_len)){
            cout << "Error: EVP_EncryptUpdate failed" << endl;
            EVP_CIPHER_CTX_free(ctx);
            return false;
        }
    ...
    if(!EVP_EncryptFinal_ex(ctx, plaintext, &len)){
        cout << "Error: EVP_EncryptFinal_ex failed" << endl;
        EVP_CIPHER_CTX_free(ctx);
        return false;
    }
    outputFile.write((char*)plaintext, len);
...
bool encryptFile(const string& name, const int mode){
    ...
    const string& outputFileName = inputFileName.substr(0, inputFileName.size() - 4) + "_" + encryptmode + ".tga";
    fstream outputFile;
    ...
    if(!encrypt(inputFile, outputFile, mode)){
...
int main(int argc, char **argv) {
    ...
    if(argc == 3){
        ...
        cout << "Error: Not enough arguments" << argc << endl;
        return 1;
    }
    else if(argc > 3){
